...
void remove(Node*& head, int oldKey) {
  ...
  while(it->next) {
    if(it->key == oldKey) break;

    prev = it;
    it = it->next;
  }

  // if one node only and valid key
  if(it == prev && it == head && it->key == oldKey) {
    delete(head);
    head = NULL;
    return;

  // if key is in head node, but not only node
  } else if(it == prev && it->key == oldKey) {
    head = head->next;
    delete(it);
    return;

  // if key is at x node, but not only node
  } else if(it->key == oldKey) {
    prev->next = it->next;
    delete(it);
    return;

  // if key is in last node, but not only node
  } else if(it->next == NULL && it->key == oldKey) {
    prev->next = NULL;
    delete(it);
    return;
...
Node* interleave(Node* list1, Node* list2){
   ...
   // when both lists are empty
    ...
    // when l1 is not empty and l2 is empty
    } else if (list1 && !list2) {
        ret_list->key = list1->key;      
        ret_list->next = interleave(list1->next, list2); 
    
    // when l1 is empty and l2 is not empty
    } else if (!list1 && list2){  
